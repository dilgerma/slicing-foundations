...
class EventsDebugController(
    ...
    fun resolveHistory(
        ...
        val mappedDomainEvents = domainEvents.map { event ->
            EventMessageInfo(
                id = event.identifier,
        ...
        if (processingGroup != null) {
            val dlq = configuration.eventProcessingConfiguration().deadLetterQueue(processingGroup).orElse(null)
            var letters: List<EventMessageInfo> = dlq.deadLetters()
                .toList().flatMap { it.asSequence() }
                ...
                .map {
                    EventMessageInfo(
                        ...
                        emptyList(),
                        ElementType.EVENT, it.second?.message() ?: "",
                        highlighted = true
                    )
                }
